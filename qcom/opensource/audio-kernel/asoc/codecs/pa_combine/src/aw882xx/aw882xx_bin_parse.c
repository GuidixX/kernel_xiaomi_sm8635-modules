...
int aw_check_register_num_v1(struct aw_bin *bin, int bin_num) {
  ...
  DBG("aw_bin_parse bin_num=%d, parse_register_num = 0x%x, check_register_num "
      "= 0x%x\n",
      bin_num, parse_register_num, check_register_num);
  if (parse_register_num != check_register_num) {
    p_check_sum = NULL;
    DBG_ERR("aw_bin_parse register num is error\n");
    DBG_ERR("aw_bin_parse bin_num=%d, parse_register_num = 0x%x, "
            "check_register_num = 0x%x\n",
            bin_num, parse_register_num, check_register_num);
    return -EINVAL;
  }
...
int aw_check_dsp_reg_num_v1(struct aw_bin *bin, int bin_num) {
  ...
  bin->header_info[bin_num].reg_data_byte_len =
      GET_32_DATA(*(p_check_sum + 11), *(p_check_sum + 10), *(p_check_sum + 9),
                  *(p_check_sum + 8));
  check_dsp_reg_num = (bin->header_info[bin_num].bin_data_len - 12) /
                      bin->header_info[bin_num].reg_data_byte_len;
  DBG("aw_bin_parse bin_num=%d, parse_dsp_reg_num = 0x%x, check_dsp_reg_num = "
      "0x%x\n",
      bin_num, parse_dsp_reg_num, check_dsp_reg_num);
  if (parse_dsp_reg_num != check_dsp_reg_num) {
    p_check_sum = NULL;
    DBG_ERR("aw_bin_parse dsp reg num is error\n");
    DBG_ERR("aw_bin_parse bin_num=%d, parse_dsp_reg_num = 0x%x, "
            "check_dsp_reg_num = 0x%x\n",
            bin_num, parse_dsp_reg_num, check_dsp_reg_num);
    return -EINVAL;
  }
  bin->header_info[bin_num].download_addr =
      GET_32_DATA(*(p_check_sum + 3), *(p_check_sum + 2), *(p_check_sum + 1),
                  *(p_check_sum));
...
int aw_check_soc_app_num_v1(struct aw_bin *bin, int bin_num) {
  ...
  bin->header_info[bin_num].app_version =
      GET_32_DATA(*(p_check_sum + 3), *(p_check_sum + 2), *(p_check_sum + 1),
                  *(p_check_sum));
  parse_soc_app_num = GET_32_DATA(*(p_check_sum + 11), *(p_check_sum + 10),
                                  *(p_check_sum + 9), *(p_check_sum + 8));
  check_soc_app_num = bin->header_info[bin_num].bin_data_len - 12;
  DBG("aw_bin_parse bin_num=%d, parse_soc_app_num = 0x%x, check_soc_app_num = "
      "0x%x\n",
      bin_num, parse_soc_app_num, check_soc_app_num);
  if (parse_soc_app_num != check_soc_app_num) {
    p_check_sum = NULL;
    DBG_ERR("aw_bin_parse soc app num is error\n");
    DBG_ERR("aw_bin_parse bin_num=%d, parse_soc_app_num = 0x%x, "
            "check_soc_app_num = 0x%x\n",
            bin_num, parse_soc_app_num, check_soc_app_num);
    return -EINVAL;
  }
  bin->header_info[bin_num].reg_num = parse_soc_app_num;
  bin->header_info[bin_num].download_addr =
      GET_32_DATA(*(p_check_sum + 7), *(p_check_sum + 6), *(p_check_sum + 5),
                  *(p_check_sum + 4));
  bin->header_info[bin_num].valid_data_len =
      bin->header_info[bin_num].bin_data_len - 12;
  p_check_sum = NULL;
...
void aw_get_single_bin_header_1_0_0(struct aw_bin *bin) {
  ...
  bin->header_info[bin->all_bin_parse_num].check_sum =
      GET_32_DATA(*(bin->p_addr + 3), *(bin->p_addr + 2), *(bin->p_addr + 1),
                  *(bin->p_addr));
  bin->header_info[bin->all_bin_parse_num].header_ver =
      GET_32_DATA(*(bin->p_addr + 7), *(bin->p_addr + 6), *(bin->p_addr + 5),
                  *(bin->p_addr + 4));
  bin->header_info[bin->all_bin_parse_num].bin_data_type =
      GET_32_DATA(*(bin->p_addr + 11), *(bin->p_addr + 10), *(bin->p_addr + 9),
                  *(bin->p_addr + 8));
  bin->header_info[bin->all_bin_parse_num].bin_data_ver =
      GET_32_DATA(*(bin->p_addr + 15), *(bin->p_addr + 14), *(bin->p_addr + 13),
                  *(bin->p_addr + 12));
  bin->header_info[bin->all_bin_parse_num].bin_data_len =
      GET_32_DATA(*(bin->p_addr + 19), *(bin->p_addr + 18), *(bin->p_addr + 17),
                  *(bin->p_addr + 16));
  bin->header_info[bin->all_bin_parse_num].ui_ver =
      GET_32_DATA(*(bin->p_addr + 23), *(bin->p_addr + 22), *(bin->p_addr + 21),
                  *(bin->p_addr + 20));
  bin->header_info[bin->all_bin_parse_num].reg_byte_len =
      GET_32_DATA(*(bin->p_addr + 35), *(bin->p_addr + 34), *(bin->p_addr + 33),
                  *(bin->p_addr + 32));
  bin->header_info[bin->all_bin_parse_num].data_byte_len =
      GET_32_DATA(*(bin->p_addr + 39), *(bin->p_addr + 38), *(bin->p_addr + 37),
                  *(bin->p_addr + 36));
...
int aw_parse_each_of_multi_bins_1_0_0(unsigned int bin_num, int bin_serial_num,
  ...
  if (!bin_serial_num) {
    bin_start_addr = GET_32_DATA(*(bin->p_addr + 67), *(bin->p_addr + 66),
                                 *(bin->p_addr + 65), *(bin->p_addr + 64));
    bin->p_addr += (60 + bin_start_addr);
    bin->header_info[bin->all_bin_parse_num].valid_data_addr =
        bin->header_info[bin->all_bin_parse_num - 1].valid_data_addr + 4 +
        8 * bin_num + 60;
  } else {
...
/* Get the number of bins in multi bins, and set a for loop, loop processing
 * each bin data */
 ...
 * If the bin framework header version is 1.0.0,
   ...
   If it is a single bin data type, write the data directly into the structure
 array If it is a multi-bin data type, first obtain the number of bins, and then
 recursively call the bin frame header processing function according to the bin
 number to process the frame header information of each bin separately
 *
 ********************************************************/
static int aw_parse_bin_header_1_0_0(struct aw_bin *bin) {
  int ret = 0;
...
/*********************************awinic audio
 * acf*************************************/
static uint8_t aw_dev_parse_crc8_check(unsigned char *data,
                                       uint32_t data_size) {
...
static int aw_dev_parse_check_acf_by_hdr(struct aw_container *aw_cfg) {
  ...
  for (i = 0; i < cfg_hdr->a_ddt_num; i++) {
    ...
    if (end_data_offset > aw_cfg->len) {
      aw_pr_err("a_ddt_num[%d] end_data_offset[%d] overflow file size[%d]", i,
                end_data_offset, aw_cfg->len);
      return -EINVAL;
    }

    /* crc check */
    act_crc8 = aw_dev_parse_crc8_check(aw_cfg->data + cfg_dde[i].data_offset,
                                       cfg_dde[i].data_size);
    if (act_crc8 != cfg_dde[i].data_crc) {
      aw_pr_err(
          "a_ddt_num[%d] crc8 check failed, act_crc8:0x%x != data_crc 0x%x", i,
          (uint32_t)act_crc8, cfg_dde[i].data_crc);
      return -EINVAL;
    }
  }
...
static int
aw_dev_parse_check_acf_by_hdr_v_1_0_0_0(struct aw_container *aw_cfg) {
  struct aw_cfg_hdr *cfg_hdr = NULL;
...
int aw882xx_dev_parse_check_acf(struct aw_container *aw_cfg) {
  ...
  if (aw_cfg->len < sizeof(struct aw_cfg_hdr)) {
    aw_pr_err("cfg hdr size[%d] overflow file size[%d]", aw_cfg->len,
...
static int aw_dev_parse_data_by_sec_type(struct aw_device *aw_dev,
  ...
  case ACF_SEC_TYPE_HDR_REG:
    return aw_dev_parse_reg_bin_with_hdr(
        aw_dev, (uint8_t *)cfg_hdr + prof_hdr->data_offset, prof_hdr->data_size,
        scene_prof_desc);
  ...
  for (i = 0; i < prof_hdr->a_ddt_num; i++) {
    if ((aw_dev->i2c->adapter->nr == cfg_dde[i].dev_bus) &&
      ...
      if (cfg_dde[i].data_type != ACF_SEC_TYPE_MONITOR) {
        ...
        ret =
            aw_dev_parse_data_by_sec_type(aw_dev, prof_hdr, &cfg_dde[i], NULL);
        if (ret < 0) {
...
static int
aw_dev_parse_dev_default_type(struct aw_device *aw_dev,
                              struct aw_cfg_hdr *prof_hdr,
                              struct aw_all_prof_info *all_prof_info) {
  int i = 0;
  int ret;
    ...
    if ((aw_dev->channel == cfg_dde[i].dev_index) &&
      ...
      if (cfg_dde[i].data_type == ACF_SEC_TYPE_DSP) {
        ret = aw_dev_parse_raw_dsp(
            aw_dev, (uint8_t *)prof_hdr + cfg_dde[i].data_offset,
            cfg_dde[i].data_size,
            &all_prof_info->prof_desc[cfg_dde[i].dev_profile]);
        if (ret < 0) {
...
static int aw_dev_parse_get_vaild_prof(struct aw_device *aw_dev,
  ...
  prof_info->prof_desc =
      kzalloc(prof_info->count * sizeof(struct aw_prof_desc), GFP_KERNEL);
  ...
  for (i = 0; i < AW_PROFILE_MAX; i++) {
    if (prof_desc[i].prof_st == AW_PROFILE_OK) {
      if (num >= prof_info->count) {
        aw_dev_err(aw_dev->dev, "get scene num[%d] overflow count[%d]", num,
                   prof_info->count);
        return -ENOMEM;
      }
...
aw_dev_parse_create_prof_name_list_v_1_0_0_0(struct aw_device *aw_dev) {
  ...
  prof_info->prof_name_list =
      devm_kzalloc(aw_dev->dev, prof_info->count * PROFILE_STR_MAX, GFP_KERNEL);
...
static int aw_dev_parse_drv_type_v_1_0_0_0(struct aw_device *aw_dev,
  ...
  case ACF_SEC_TYPE_REG:
    ret = aw_dev_parse_raw_reg(
        aw_dev, (uint8_t *)prof_hdr + cfg_dde->data_offset, cfg_dde->data_size,
...
static int aw_dev_parse_dev_type_v_1_0_0_0(struct aw_device *aw_dev,
  ...
  for (i = 0; i < prof_hdr->a_ddt_num; i++) {
    if ((cfg_dde[i].type == AW_DEV_TYPE_ID) &&
        ...
        (aw_dev->i2c->addr == cfg_dde[i].dev_addr) &&
        (aw_dev->chip_id == cfg_dde[i].chip_id)) {
      ret = aw_dev_parse_drv_type_v_1_0_0_0(aw_dev, prof_hdr, &cfg_dde[i],
                                            &cur_scene_id);
      if (ret < 0)
        return ret;
    }
...
static int
aw_dev_parse_dev_default_type_v_1_0_0_0(struct aw_device *aw_dev,
                                        struct aw_cfg_hdr *prof_hdr) {
  int i = 0;
  int ret;
  int cur_scene_id = 0;
...
static int aw_dev_parse_acf_by_hdr_v_1_0_0_0(struct aw_device *aw_dev,
  ...
  prof_info->prof_desc = devm_kzalloc(
      aw_dev->dev, prof_info->count * sizeof(struct aw_prof_desc), GFP_KERNEL);
  if (prof_info->prof_desc == NULL) {
...
int aw88xx_dev_get_profile_name(struct aw_device *aw_dev, char *name,
  ...
  if (index > prof_info->count) {
    aw_dev_err(aw_dev->dev, "index[%d] overflow dev prof num[%d]", index,
               prof_info->count);
    return -EINVAL;
  }
